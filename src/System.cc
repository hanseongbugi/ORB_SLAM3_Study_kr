    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    //Check settings file
    cv::FileStorage fsSettings(strSettingsFile.c_str(), cv::FileStorage::READ); // yaml 파일 읽기
    if(!fsSettings.isOpened()) // 열수 없다면 에러 표시 후 종료
    {
    ...
    node = fsSettings["loopClosing"]; // loopClosing 관련 인자 가져오기 (없어도 디폴트가 활성화)
    bool activeLC = true;
    if(!node.empty()) // loopClosing인자가 있다면
    {
        activeLC = static_cast<int>(fsSettings["loopClosing"]) != 0; // 0이 아니라면 LoopClosing 활성화
    }

    mStrVocabularyFilePath = strVocFile; // Vocabulary 파일 경로 저장

    bool loadedAtlas = false;

    if(mStrLoadAtlasFromFile.empty()) // 아틸라스 파일이 없다면
    {
        //Load ORB Vocabulary
        cout << endl << "Loading ORB Vocabulary. This could take a while..." << endl;

        mpVocabulary = new ORBVocabulary(); // Vocabulary 객체 생성
        bool bVocLoad = mpVocabulary->loadFromTextFile(strVocFile); // Vocabulary 파일 경로를 알려줌 
        if(!bVocLoad)
        {
        ...
        mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary); // keyFrameDatabase 객체 생성

        //Create the Atlas
        cout << "Initialization of Atlas from scratch " << endl;
        mpAtlas = new Atlas(0); // Atlas 객체 생성
    }
    else
    {
        //Load ORB Vocabulary
    ...
    if (mSensor==IMU_STEREO || mSensor==IMU_MONOCULAR || mSensor==IMU_RGBD)
        mpAtlas->SetInertialSensor(); // Map 객체에 센서를 사용한다고 알림

    //Create Drawers. These are used by the Viewer
    mpFrameDrawer = new FrameDrawer(mpAtlas); // Drawer 객체 생성 (Viewer에서 사용)
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpAtlas, mpKeyFrameDatabase, strSettingsFile, mSensor, settings_, strSequence); // Tracking 객체 생성

    //Initialize the Local Mapping thread and launch
    mpLocalMapper = new LocalMapping(this, mpAtlas, mSensor==MONOCULAR || mSensor==IMU_MONOCULAR,
                                     mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO || mSensor==IMU_RGBD, strSequence); // LocalMapping 객체 생성
    mptLocalMapping = new thread(&ORB_SLAM3::LocalMapping::Run,mpLocalMapper); // LocalMapping 객체를 별도의 스레드에서 실행
    ...
    mptLoopClosing = new thread(&ORB_SLAM3::LoopClosing::Run, mpLoopCloser); // LoopingClosing 객체 생성 후 별도의 스레드에서 실행

    //Set pointers between threads
    mpTracker->SetLocalMapper(mpLocalMapper); // 각각의 스레드에 객체의 포인터를 전달
    ...
    if(bUseViewer) // Viewer 사용 시 Viewer 객체 생성
    //if(false) // TODO
    {
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbShutDown) // 프로그램 ShutDown 시
            return Sophus::SE3f(); // 빈 객체 생성 후 반환
    }

    if(mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR) // MonoCular가 아닌 경우
    {
        cerr << "ERROR: you called TrackMonocular but input sensor was not set to Monocular nor Monocular-Inertial." << endl;
        exit(-1); // 에러 표시 후 프로그램 종료
    }

    cv::Mat imToFeed = im.clone(); // 이미지 객체 복사
    if(settings_ && settings_->needToResize()){ // 이미지 크기 변환이 필요한 경우 resize
        cv::Mat resizedIm;
        cv::resize(im,resizedIm,settings_->newImSize());
        imToFeed = resizedIm;
    }
    ...
    {
        unique_lock<mutex> lock(mMutexMode);
        if(mbActivateLocalizationMode) // LocalMapping이 실행 중인 경우
        {
            mpLocalMapper->RequestStop(); // LocalMapping 스레드에 중단 요청

            // Wait until Local Mapping has effectively stopped
            while(!mpLocalMapper->isStopped()) // 중단될 때 까지 대기
            {
                usleep(1000);
            }

            mpTracker->InformOnlyTracking(true); // Tracking 객체에 Tracking만 실행되고 있다고 알려줌
            mbActivateLocalizationMode = false;
        }
        if(mbDeactivateLocalizationMode) // LocalMapping이 실행 중이지 않는 경우
        {
            mpTracker->InformOnlyTracking(false); // Tracking 객체에 Tracking만 실행되고 있지 않다고 알려줌
    ...
    {
        unique_lock<mutex> lock(mMutexReset);
        if(mbReset) // 리셋 플레그 활성화 시
        {
            mpTracker->Reset(); // Tracking 리셋
            mbReset = false;
            mbResetActiveMap = false;
        }
        else if(mbResetActiveMap) // Map 리셋 플레그 활성화 시
        {
            cout << "SYSTEM-> Reseting active map in monocular case" << endl;
            mpTracker->ResetActiveMap(); // 활성화된 맵 리셋
            mbResetActiveMap = false;
        }
    }
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]); // Imu 데이터를 가져와 Tracking 객체에 저장 

    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename); // Tracking객체에 이미지 전달

    unique_lock<mutex> lock2(mMutexState);
    mTrackingState = mpTracker->mState; // 현재 Tracking 상태 저장
    mTrackedMapPoints = mpTracker->mCurrentFrame.mvpMapPoints; // 현재 Frame의 MapPoint 배열 저장
    mTrackedKeyPointsUn = mpTracker->mCurrentFrame.mvKeysUn; // 현재 Frame의 keyPoint 배열 저장
