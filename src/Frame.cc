...
{
    // Frame ID
    mnId=nNextId++; // Frame ID 증가

    // Scale Level Info
    // ORB 추출 객체 생성 시 저장한 값을 저장 (scale Level 정보, Scale Factor 정보 등 
    mnScaleLevels = mpORBextractorLeft->GetLevels(); 
    mfScaleFactor = mpORBextractorLeft->GetScaleFactor();
    mfLogScaleFactor = log(mfScaleFactor);
    mvScaleFactors = mpORBextractorLeft->GetScaleFactors();
    ...
    std::chrono::steady_clock::time_point time_StartExtORB = std::chrono::steady_clock::now();
#endif
    ExtractORB(0,imGray,0,1000); // ORB 특징점 추출
#ifdef REGISTER_TIMES
    std::chrono::steady_clock::time_point time_EndExtORB = std::chrono::steady_clock::now();
    ...
    N = mvKeys.size(); // 특징점 배열의 크기 저장
    if(mvKeys.empty()) // 비어있다면 return
        return;

    UndistortKeyPoints(); // 왜곡된 keyPoint 판단

    // Set no stereo information
    mvuRight = vector<float>(N,-1); // Mono인 경우 사용할 값들 초기화
    ...
    mvbOutlier = vector<bool>(N,false); // 아웃라이너 판단 배열 생성

    // This is done only for the first Frame (or after a change in the calibration)
    if(mbInitialComputations) // 첫 프레임인 경우 
    {
        ComputeImageBounds(imGray); // 이미지 경계 값 계산

        mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);
        mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/static_cast<float>(mnMaxY-mnMinY);

        fx = static_cast<Pinhole*>(mpCamera)->toK().at<float>(0,0); // 카메라 계수 저장 
    ...
    AssignFeaturesToGrid(); // 특징점을 grid로 assign 진행

    if(pPrevF) // 이전 Frame이 존재하고
    {
        if(pPrevF->HasVelocity()) // 이전 프레임이 속도 정보를 가지고 있다면
        {
            SetVelocity(pPrevF->GetVelocity()); // 이전 프레임의 속도를 현재 frame의 속도로 설정
        }
    }
    else
    {
        mVw.setZero(); // 없다면 0으로 설정
    }
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    if(flag==0)
        monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping); // ORB 특징점 추출 후 특징점 계수 저장
    else
...
{
    if(mDistCoef.at<float>(0)!=0.0) // 왜곡 계수가 0이 아닌 경우
    {
        ...
        mat=mat.reshape(2); // 2채널 행렬로 변경
        cv::undistortPoints(mat,mat,static_cast<Pinhole*>(mpCamera)->toK(),mDistCoef,cv::Mat(),mK); // 왜곡을 제거한 Matrix로 변경
        mat=mat.reshape(1); // 1채널 행렬로 변경

        // Undistort corners
        mnMinX = min(mat.at<float>(0,0),mat.at<float>(2,0)); // 왜곡 제거된 코너 추출
