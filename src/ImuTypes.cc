...
{
    mvMeasurements.push_back(integrable(acceleration,angVel,dt)); // 가속도, 각속도, 시간차이 값을 integrable 객체로 생성해 배열에 저장
    ...
    A.setIdentity(); // 단위 행렬로 생성
    Eigen::Matrix<float,9,6> B;
    B.setZero(); // 0 행렬로 생성

    Eigen::Vector3f acc, accW;
    acc << acceleration(0)-b.bax, acceleration(1)-b.bay, acceleration(2)-b.baz; // ax, ay, az 값을 bias와 빼서 배열에 저장
    accW << angVel(0)-b.bwx, angVel(1)-b.bwy, angVel(2)-b.bwz; // gx, gy, gz 값을 bias와 빼서 배열에 저장

    // 누적된 시간 간격 dT와 현재 시간 간격 dt에 대한 평균 가속도와 각속도를 갱신
    avgA = (dT*avgA + dR*acc*dt)/(dT+dt); //dR * acc * dt는 현재 시간 간격 동안의 변화된 가속도 (평균 가속도에 새로운 시간 간격 동안의 가속도 변화를 가중 평균을 통해 평균 가속도 갱신)
    avgW = (dT*avgW + accW*dt)/(dT+dt); // accW * dt는 현재 시간 간격 동안의 각속도 변화 (이전까지의 평균 각속도에 새로운 시간 간격 동안의 각속도 변화를 가중 평균하여 갱신)

    // Update delta position dP and velocity dV (rely on no-updated delta rotation)
    dP = dP + dV*dt + 0.5f*dR*acc*dt*dt; //현재 위치 갱신. 이전 위치에 현재 속도와 가속도의 변화를 더함으로써 갱신
    dV = dV + dR*acc*dt; //현재 속도 갱신. 이전 속도에 현재 가속도와 회전에 의한 가속도의 변화를 더함으로써 갱신

    // Compute velocity and position parts of matrices A and B (rely on non-updated delta rotation)
    Eigen::Matrix<float,3,3> Wacc = Sophus::SO3f::hat(acc);

    A.block<3,3>(3,0) = -dR*dt*Wacc; // 위치에 대한 정보 저장
    A.block<3,3>(6,0) = -0.5f*dR*dt*dt*Wacc; // 위치에 대한 2차 정보 저장
    A.block<3,3>(6,3) = Eigen::DiagonalMatrix<float,3>(dt, dt, dt); //속도에 대한 정보
    B.block<3,3>(3,3) = dR*dt; // 속도에 대한 정보
    B.block<3,3>(6,3) = 0.5f*dR*dt*dt; // 속도에 대한 2차 정보


    // Update position and velocity jacobians wrt bias correction
    // 위치와 속도에 대한 jacobian 행렬을 갱신 (이때 bias와 회전 행렬, 가속도 행렬 사용)
    ...
    dR = NormalizeRotation(dR*dRi.deltaR); // 회전 행렬 Normalization
    ...
    JRg = dRi.deltaR.transpose()*JRg - dRi.rightJ*dt; // 회전 jacobian 행렬 갱신 
...
// 일반적으로 Tbc, 자이로 노이즈, 가속도 노이즈, 자이로 random walk, 가속도 random walk를 이용해 Calibration 객체 생성 시 호출
void Calib::Set(const Sophus::SE3<float> &sophTbc, const float &ng, const float &na, const float &ngw, const float &naw) {
    mbIsSet = true;
    const float ng2 = ng*ng; // 노이즈들을 제곱하여 저장
    const float na2 = na*na;
    const float ngw2 = ngw*ngw;
    const float naw2 = naw*naw;

    // Sophus/Eigen
    mTbc = sophTbc; // Tbc 저장 (Imu to Cam)
    mTcb = mTbc.inverse(); // Cam to Imu 행렬 저장
    Cov.diagonal() << ng2, ng2, ng2, na2, na2, na2; // 노이즈를 대각 행렬에 저장
    CovWalk.diagonal() << ngw2, ngw2, ngw2, naw2, naw2, naw2; // random walk를 대각 행렬에 저장
}
